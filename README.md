...
- 生产必须接入 OpenClaw 原生回调/插件桥接，再调用 `/Users/shift/openclaw/scripts/ops_approval_center.py apply-callback`。
- `/Users/shift/openclaw/scripts/ops_telegram_approval_watcher.py` 是禁用的遗留 smoke 工具；不要启动它，它会和 OpenClaw 默认 Telegram bot 的 `getUpdates` 长轮询冲突，导致 NestCar 等独立 agent 已读不回。
...
ls /Users/shift/openclaw/ops/approvals/approved
```
...
curl -X POST http://127.0.0.1:8765/v1/employees/onboard \
  -H 'Content-Type: application/json' \
  --data '{"id":"api-reviewer","name":"API Reviewer","role":"reviewer","runtime":"hermes","workspace":"/Users/shift/openclaw/company-kernel/employees/api-reviewer","alias":"api-reviewer","skills":"review,qa","create_test_task":"true"}'
