...
class VerifierAccuracyTest(unittest.TestCase):
    ...
    def test_human_review_links_to_latest_unreviewed_run_for_task(self):
        rec(self.conn, "t1", "numeric", "pass")
        companyctl.link_human_review_to_verifier(self.conn, "t1", "accepted", "owner-shift")
    ...
    def test_pass_rejected_is_false_positive(self):
        rec(self.conn, "t1", "numeric", "pass")
        companyctl.link_human_review_to_verifier(self.conn, "t1", "rejected", "owner-shift")
        acc = companyctl.compute_verifier_accuracy(self.conn)
    ...
    def test_link_noop_when_no_run_or_bad_status(self):
        ...
        companyctl.link_human_review_to_verifier(self.conn, "missing", "accepted", "owner-shift")
        rec(self.conn, "t1", "numeric", "pass")
        companyctl.link_human_review_to_verifier(self.conn, "t1", "draft", "owner-shift")  # invalid status
