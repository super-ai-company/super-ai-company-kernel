...
- 远端 `/home/happy/openclaw-company-management` 与本地核心文件 hash 一致：
  ...
  - `tests/test_skill.sh`
- 远端 `/home/happy/openclaw-company-management/install.sh` 与本地不一致；远端有未提交修改，主要把默认工作区从本机 `/Users/shift/openclaw/workspace-xmanx` 改成 `$HOME/openclaw/workspace-main`，并用 Python sqlite3 初始化 schema。
