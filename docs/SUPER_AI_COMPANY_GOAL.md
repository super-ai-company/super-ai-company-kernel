...
- 独立项目目录：`/Users/shift/openclaw/company-kernel`
...
- OpenClaw adapter 的 `--execute` 已接入 approval gate：未批准不会写 legacy bus，会生成 pending approval；批准后重跑才提交到 `/Users/shift/openclaw/ops/agent_bus`。
